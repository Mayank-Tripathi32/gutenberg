...
-   Only offer "Remove" in the "This block can only be used once" warning when the block can actually be removed. A block that is both `supports.multiple: false` and locked against removal is handed no `onReplace`, so the button threw a TypeError instead of removing anything.
